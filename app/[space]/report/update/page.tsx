...
import { getAgent, getReportsByAgent, getSpace } from "@/sanity/sanity-utils";
...
const page = async ({ params }: any) => {
  ...
  const reports = await getReportsByAgent(spaceDetails[0].id, sessionName);
  ...
  return (
    <>
      ...
      <main className="allactant">
        <h1>Update feedbacks in the network</h1>
        <p>Please note that you can only update the feedback you care for</p>
        <form action={`${space}/report`}>
          <button className="button primary">Back to All Feedback</button>
        </form>
          ...
          {reports.map((report: ReportType) => (
            <div className="actants_scroller update" key={report.name}>
              ...
              <div>
                ...
                <form
                  className="function_button"
                  action={`/${space}/report/remove/${report.slug}`}
                >
